...
//        }
        ...
        {
            $class = $m_point->read_class_by_id_teacher($_SESSION['user_teacher']->ma_gv);
        }

        if(!empty($_POST["ma_lop"]) || !empty($_GET['ma_lop']))
        {
            if(!empty($_POST['ma_lop'])) {
                $ma_lop = $_POST['ma_lop'];
            }
            else {
                $ma_lop = $_GET['ma_lop'];
...
//        PHPExcel_IOFactory::createWriter($excel, 'Excel2007')->save('php://output');
    ...
    public function ImportExcel()
    {
        if(isset($_POST['btnGui']))
        {
            ...
            header("location:http://localhost:8888/cedu/teacher/point.php?ma_lop=".$ma_lop);
